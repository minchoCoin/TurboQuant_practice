...
$$
\mathrm{Var}(z_1)\le\frac{\pi}{2}\mathbb{E}\![(\mathbf{s}_1^\top\mathbf{y})^2]
...
1. $\mathbf{y}arrow \mathbf{\Pi}\mathbf{x}$
2. 媛?$j\in[d]$?????$\mathrm{idx}_jarrow \arg\min_{k\in[2^b]}|y_j-c_k|$
3. ?몃뜳??踰≫꽣 $\mathrm{idx}$ ???
蹂듭썝 $\mathrm{DeQuant}_{\tt mse}(\mathrm{idx})$:

1. 媛?$j$?????$\tilde y_jarrow c_{\mathrm{idx}_j}$
2. $\tilde{\mathbf{x}}arrow \mathbf{\Pi}^\top\tilde{\mathbf{y}}$
3. $\tilde{\mathbf{x}}$ 異쒕젰
